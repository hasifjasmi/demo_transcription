...
import { Mic, MicOff, Trash2, Download, Clock, Volume2, AlertCircle, CheckCircle2 } from 'lucide-react';
...
const SpeechTranscriptionApp = () => {
  ...
  const updateStats = () => {
    if (transcripts.length > 0) {
      const totalWords = transcripts.reduce((sum, t) => sum + (t.word_count || t.text.split(' ').length), 0);
  ...
  const connectToTranscription = () => {
    ...
    try {
      eventSourceRef.current = new EventSource('http://localhost:8000/transcribe');
      ...
      eventSourceRef.current.onerror = (event) => {
        setError('Failed to connect to transcription service. Make sure your API server is running on localhost:8000');
  ...
  const exportTranscripts = () => {
    const text = transcripts.map(t => 
      `[${t.timestamp}] Speaker ${t.speaker} (${(t.confidence * 100).toFixed(1)}% confidence, ${t.duration?.toFixed(2)}s): ${t.text}`
    ).join('\n');
    const blob = new Blob([text], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `transcription-enhanced-${new Date().toISOString().split('T')[0]}.txt`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-2">
          <h1 className="text-3xl font-bold text-slate-800">Enhanced Live Speech Transcription</h1>
          <p className="text-slate-600">Real-time speech-to-text with timestamps, confidence scores & endpoint detection</p>
        </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold">{formatTime(stats.sessionDuration)}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold">{Object.keys(speakerColors).length}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <CardContent className="p-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <Button
                  onClick={isConnected ? disconnectFromTranscription : connectToTranscription}
                  disabled={isLoading}
                ...
                >
                  ...
                  {isLoading ? 'Connecting...' : isConnected ? 'Stop Transcription' : 'Start Transcription'}
                </Button>
        ...
        <Card>
          ...
          <CardContent>
            <ScrollArea ref={scrollAreaRef} className="h-96 w-full rounded-md border p-4">
              {transcripts.length === 0 && Object.keys(partialTranscripts).length === 0 ? (
                <div className="flex items-center justify-center h-full text-slate-500">
                  {isConnected ? (
                    <div className="text-center space-y-2">
                      <Mic className="w-8 h-8 mx-auto text-slate-400" />
                      <p>Listening for speech...</p>
                      <p className="text-xs">Enhanced with timestamps, confidence scores & custom vocabulary</p>
                    </div>
                  ) : (
                    <div className="text-center space-y-2">
                      <MicOff className="w-8 h-8 mx-auto text-slate-400" />
                      <p>Click "Start Transcription" to begin</p>
                      <p className="text-xs">Features: Timestamps • Confidence • Custom Vocabulary • Endpoint Detection</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="space-y-3">
                  {/* Show partial transcripts first (real-time) */}
                  {Object.entries(partialTranscripts).map(([speaker, partial]) => (
                    <div key={`partial-${speaker}`} className="flex items-start gap-3 opacity-70 border-l-2 border-yellow-300 pl-3">
                      <Badge className={`${speakerColors[speaker]} shrink-0 font-mono text-xs`}>
                        Speaker {speaker}
                      </Badge>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <div className="flex items-center gap-1 text-xs text-slate-500">
                            ...
                            <span className={getConfidenceColor(partial.confidence)}>
                              {(partial.confidence * 100).toFixed(0)}%
                            </span>
                          </div>
                        </div>
                        <p className="text-slate-700 leading-relaxed break-words italic">
                          {partial.text}...
                        </p>
                  ...
                  {transcripts.map((transcript) => (
                    <div key={transcript.id} className="flex items-start gap-3 group">
                      <Badge 
                        className={`${speakerColors[transcript.speaker]} shrink-0 font-mono text-xs`}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {transcript.duration && (
                            <div className="flex items-center gap-1 text-xs text-slate-500">
                              ...
                              <span>{transcript.duration.toFixed(1)}s</span>
                            </div>
                          )}
                          {transcript.type === 'segment' && (
                        ...
                        <div className="flex items-center gap-4 text-xs text-slate-500 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <span>{transcript.timestamp}</span>
                          {transcript.start_time !== undefined && (
                            <span>
                              {transcript.start_time.toFixed(1)}s - {transcript.end_time.toFixed(1)}s
                            </span>
                          )}
            ...
            <div className="text-sm text-slate-600 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <ul className="list-disc list-inside space-y-1 ml-2">
                  <li><strong>Real-time Display:</strong> See partial transcripts as you speak</li>
                  <li><strong>Confidence Scores:</strong> Visual indicators for transcription accuracy</li>
                  <li><strong>Timestamps:</strong> Precise timing for each speech segment</li>
                  <li><strong>Custom Vocabulary:</strong> Enhanced recognition for "Titian System Solution"</li>
                </ul>
                <ul className="list-disc list-inside space-y-1 ml-2">
                  <li><strong>Endpoint Detection:</strong> Automatic speech start/end detection</li>
                  <li><strong>Low Latency:</strong> ~1 second maximum delay</li>
                  <li><strong>Speaker Tracking:</strong> Improved speaker diarization</li>
                  <li><strong>Session Stats:</strong> Word count and confidence metrics</li>
                </ul>
              </div>
              
              <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                <p className="text-sm"><strong>Custom Terms Optimized:</strong></p>
              ...
              <div className="mt-4 p-3 bg-green-50 rounded-lg">
                <p className="text-sm"><strong>Confidence Legend:</strong></p>
